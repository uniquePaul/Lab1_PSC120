	...
	public Agent findLocalDate(Environment state) {
		...
		else 
		{
			for (int i = 0; i < neighbors.toArray().length; i++) {
				...
				if ((((Agent)neighbors.toArray()[i]).female != (((Agent)b.toArray()[0]).female)) 
						&&
						(((Agent)neighbors.toArray()[i]).dated==false)) {	
					return (Agent)neighbors.toArray()[i];
				}
			}
			return null;
		}//a place holder for when you will return an agent or null if none can be found
	}
